...
def compute_md5_hash(string: str) -> str:
...
def brute_force(source_hash: str, length_pass: int, alphabet: list) -> float:
    ...
    for i in range(length_pass ** len(alphabet)):
        ...
        if source_hash == compute_md5_hash(prob_password):
...
if __name__ == "__main__":
    ...
    alphabet = "abekpsw123"
    source_pass = 'weakp'
    length_pass = len(source_pass)

    print(f"SOURCE_PASS: {source_pass}")
    print(f"TIME BRUTTING: {brute_force(compute_md5_hash(source_pass), length_pass, alphabet)} s")
